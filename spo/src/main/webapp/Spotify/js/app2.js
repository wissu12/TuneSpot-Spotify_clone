...
let audioElement = new Audio('songs_spo/0.mp3');


let masterPlay = document.getElementById('masterplay');
let songItems = Array.from(document.getElementsByClassName('songItem2'));
let wave =document.getElementsByClassName('wave')[0];


songItems.forEach((element, i)=>{ 
    element.getElementsByTagName("img")[0].src = songs2[i].coverPath; 
    element.getElementsByClassName("songName2")[0].innerText = songs2[i].songName; 
})
...
.forEach((element)=>{
    element.addEventListener('click',(e)=>{
        ...
        makeAllBackgrounds();
        Array.from(document.getElementsByClassName('songItem2'))[`${index}`].style.background="rgb(105,105,170,.1";
...
back.addEventListener('click',()=>{
    index-=1;
    if(index<1){
        index.Array.from(document.getElementsByClassName('songItem2')).length;
    }
    audioElement.src = `songs_spo/${index}.mp3`;
        poster_master_play.src=`image_songs/${index}.jpg`;
        audioElement.play();
...
next.addEventListener('click',()=>{
    index-=0;
    index+=1;
    if(index>Array.from(document.getElementsByClassName('songItem2')).length){
        index=1;
    }
    audioElement.src = `songs_spo/${index}.mp3`;
        poster_master_play.src=`image_songs/${index}.jpg`;
